...
import { v2Campaign, memReplayJournal } from "./policy-test-fixtures";
...
const campaign = (): Campaign => v2Campaign();
...
describe("payout replay idempotency", () => {
  it("a completed reproduced result is REUSED on retry (no second browser run)", async () => {
    ...
    let runs = 0;
    const deps: PayoutReplayDeps = { submissionId: "sub-1", journal: j, runProbe: async (p) => { runs++; return { classification: "reproduced" as ProbeClassification, reason: "", probeId: p.id }; } };
  ...
  it("a completed VETO is reused on retry (still holds, no re-run)", async () => {
    ...
    const j = memReplayJournal();
    let runs = 0;
    const deps: PayoutReplayDeps = { submissionId: "sub-2", journal: j, runProbe: async (p) => { runs++; return { classification: "no_observable_change" as ProbeClassification, reason: "", probeId: p.id }; } };
  ...
  it("a DIFFERENT submission is a different key → a fresh run", async () => {
    ...
    const j = memReplayJournal();
    let runs = 0;
    const mk = (sid: string): PayoutReplayDeps => ({ submissionId: sid, journal: j, runProbe: async (p) => { runs++; return { classification: "reproduced" as ProbeClassification, reason: "", probeId: p.id }; } });
  ...
  it("an IN-FLIGHT crash (begun, never completed) reconciles by re-running", async () => {
    ...
    j.begin("sub-3", campaign().verificationPolicyDigest!, (campaign().verificationPolicy as { probes: { probeDigest: string }[] }).probes[0].probeDigest);
    let runs = 0;
    const deps: PayoutReplayDeps = { submissionId: "sub-3", journal: j, runProbe: async (p) => { runs++; return { classification: "reproduced" as ProbeClassification, reason: "", probeId: p.id }; } };
