...
const RAW = {
  checkpoints: [
    ...
    {
      kind: "state",
      requirement: "Pass the intro/onboarding",
      targetEntity: "",
      requiredContext: "",
      dependsOnIndexes: [1],
      sourcePhrase: "land in example.test",
    ...
    {
      kind: "navigation",
      requirement: "Reach the main area",
      targetEntity: "",
      requiredContext: "main area",
      dependsOnIndexes: [2],
      sourcePhrase: "go to the guide character",
    },
    {
      kind: "navigation",
      requirement: "Locate the guide character in the main area",
      targetEntity: "guide",
      requiredContext: "main area",
      dependsOnIndexes: [3],
      sourcePhrase: "go to the guide character",
...
const step = (
  over: Partial<JourneyStep> & { stateIndex: number },
): JourneyStep => ({
  actionKind: "click",
  actedLabel: "",
  stateText: "",
  ...
  factIds: [`f${over.stateIndex}`],
  transitionId: `t${over.stateIndex}`,
  ...over,
});
...
describe("compileJourneyFromRaw — Sage mints identity + order, never the model", () => {
  it("compiles an ordered journey preserving every stated action and the final outcome", () => {
    ...
    expect(j.checkpoints.map((c) => c.kind)).toEqual([
      "entry",
      "state",
      "navigation",
      "navigation",
    ...
    expect(outcomeCheckpoints(j).map((c) => c.requirement)).toEqual([
      "Observe the response",
    ]);
    for (const c of j.checkpoints)
      expect(c.checkpointId).toMatch(/^cp\d+_[0-9a-f]{8}$/); // Sage-minted
      ...
      checkpoints: [
        {
          ...
          requiredContext: "",
          dependsOnIndexes: [],
          sourcePhrase: "buy a subscription",
        },
        {
          kind: "outcome",
          requirement: "Reply",
  ...
  it("keeps the locate/open/send/reply checkpoints unmet when the name only appears on the intro screen", () => {
    const j = evaluateJourney(journey(), [
      step({
        stateIndex: 0,
        actionKind: "load",
        stateText: "Welcome — guide is waiting inside. tap to step inside",
      }),
      ...
      step({
        ...
        stateText: "intro: your guide will meet you soon",
        addedText: "your guide will meet you soon",
      }),
    ]);
    const s = Object.fromEntries(
      j.checkpoints.map((c) => [c.requirement, c.status]),
    );
  ...
  it("requires the founder's CONTEXT: the same entity click outside the required context does not count", () => {
    const j = evaluateJourney(journey(), [
      ...
      step({
        ...
        actionKind: "click",
        actedLabel: "guide",
        stateText: "settings panel",
        addedText: "settings panel",
      }),
    ]);
...
describe("#2 dependencies are structural — no out-of-order completion", () => {
  it("a late-journey observation cannot complete while its prerequisites are unmet", () => {
    const j = evaluateJourney(journey(), [
      ...
      step({
        ...
        stateText: "the guide says hello",
        addedText: "the guide says hello",
      }),
    ]);
    expect(j.checkpoints[6].status).toBe("unmet");
    expect(j.checkpoints.filter((c) => c.status === "observed")).toHaveLength(
      0,
    ); // not even entry (it was not a load)
  });
  ...
  it("nextUnmetCheckpoint always returns the first dependency-ready checkpoint", () => {
    ...
    const after = evaluateJourney(j, [
      step({ stateIndex: 0, actionKind: "load", stateText: "welcome" }),
    ]);
    expect(nextUnmetCheckpoint(after)?.requirement).toBe(
      "Pass the intro/onboarding",
    );
...
describe("#3/#4 opening a conversation is not talking; sending is not receiving", () => {
  const upToOpen = (): JourneyStep[] => [
    ...
    step({
      ...
      actionKind: "click",
      actedLabel: "come in",
      stateText: "the main area map",
      addedText: "the main area map",
    }),
    step({
      stateIndex: 3,
      actionKind: "click",
      actedLabel: "meet the guide",
      stateText: "the main area — guide",
      addedText: "guide",
    }),
    step({
      stateIndex: 4,
      actionKind: "click",
      actedLabel: "guide",
      stateText: "conversation with guide",
      addedText: "conversation with guide",
    }),
  ];
  ...
  it("a real reply (new content after the send) DOES complete the outcome, with cited evidence", () => {
    const j = evaluateJourney(journey(), [
      ...
      step({
        ...
        actionKind: "observe_response",
        stateText: "conversation",
        addedText: "warm greetings traveller",
        factIds: ["fR"],
        transitionId: "tR",
      }),
    ]);
...
describe("#5 a truthful but PARTIAL (prerequisite-only) plan is blocked", () => {
  it("an onboarding-only mission is rejected as prerequisite_only_plan / goal_outcome_uncovered", () => {
    const onboardingOnly = mission({
      ...
      instructions:
        "Open the product and click through the intro until the main area appears.",
  ...
  it("blocks with goal_checkpoint_unobserved when the browser only reached onboarding", () => {
    ...
    const r = checkJourneyCoverage(partial, [
      mission({
        title: "Complete onboarding",
        criteria: ["reaches the intro"],
      }),
    ]);
    expect(r.ok).toBe(false);
    expect(r.rejections.map((x) => x.code)).toContain(
      "goal_checkpoint_unobserved",
    );
...
describe("coverage matching is phrasing-robust but still strict about the outcome", () => {
  it("a differently-WORDED mission still covers a checkpoint ('Navigate to X' covers 'Open the product')", () => {
    const j = evaluateJourney(
      compileJourneyFromRaw("open the shop and buy nothing", {
        checkpoints: [
          {
            kind: "entry",
            requirement: "Open the product",
  ...
  it("but a mission about a DIFFERENT step never covers the outcome", () => {
    const r = checkJourneyCoverage(fullyObserved(), [
      mission({
        title: "Reach the main area",
        instructions: "Navigate to the main area of the product.",
      }),
    ]);
    expect(r.ok).toBe(false);
    expect(r.rejections.map((x) => x.code)).toContain("goal_outcome_uncovered");
...
describe("#7 unrelated product journeys behave correctly", () => {
  const DRAW_GOAL =
    "let users open the whiteboard and draw a rectangle on the canvas";
  const drawJourney = () =>
    compileJourneyFromRaw(DRAW_GOAL, {
      checkpoints: [
        {
          kind: "entry",
          requirement: "Open the whiteboard",
          targetEntity: "whiteboard",
          requiredContext: "",
          dependsOnIndexes: [],
          sourcePhrase: "open the whiteboard",
        },
        {
          kind: "interaction",
          requirement: "Select the rectangle tool",
          targetEntity: "rectangle",
          requiredContext: "",
          dependsOnIndexes: [1],
          sourcePhrase: "draw a rectangle",
        },
        {
          kind: "outcome",
          requirement: "A rectangle appears on the canvas",
          targetEntity: "rectangle",
          requiredContext: "canvas",
          dependsOnIndexes: [2],
          sourcePhrase: "draw a rectangle on the canvas",
        },
      ],
    })!;
  ...
  it("a canvas/drawing journey completes from real drag evidence (no conversation involved)", () => {
    const j = evaluateJourney(drawJourney(), [
      step({
        stateIndex: 0,
        actionKind: "load",
        stateText: "whiteboard canvas",
      }),
      step({
        stateIndex: 1,
        actionKind: "click",
        actedLabel: "rectangle",
        stateText: "whiteboard canvas",
        addedText: "rectangle selected",
      }),
      step({
        stateIndex: 2,
        actionKind: "drag",
        stateText: "whiteboard canvas — shape",
        addedText: "stroke fill opacity",
      }),
    ]);
    expect(j.checkpoints.map((c) => c.status)).toEqual([
      "observed",
      "observed",
      "observed",
    ]);
  ...
  it("a DOM signup journey (no entity, no conversation) is unaffected", () => {
    ...
    const j = compileJourneyFromRaw(g, {
      checkpoints: [
        {
          kind: "entry",
          requirement: "Open the site",
          targetEntity: "",
          requiredContext: "",
          dependsOnIndexes: [],
          sourcePhrase: "complete the signup",
        },
        {
          kind: "input",
          requirement: "Fill the signup form",
          targetEntity: "",
          requiredContext: "",
          dependsOnIndexes: [1],
          sourcePhrase: "complete the signup",
        ...
        {
          kind: "outcome",
          requirement: "The welcome screen appears",
          targetEntity: "welcome",
          requiredContext: "",
          dependsOnIndexes: [2],
          sourcePhrase: "reach the welcome screen",
        },
      ],
    })!;
    const done = evaluateJourney(j, [
      ...
      step({
        ...
        actionKind: "click",
        actedLabel: "continue",
        stateText: "welcome aboard",
        addedText: "welcome aboard",
      }),
    ]);
