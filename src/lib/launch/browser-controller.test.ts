...
const el = (over: Partial<MintedElement> & { id: string }): MintedElement => ({
  label: "", role: "", tag: "button", typable: false, ...over,
});
...
describe("affordanceRank — general forward-navigation affordances", () => {
  ...
  it("does NOT treat decorations as forward affordances", () => {
    for (const junk of ["🔊", "+", "−", "·", "make a wish at the wishing tree", "settings"]) {
      expect(affordanceRank(junk)).toBe(-1);
    }
  });
  it("prefers the most-specific intent (get started > start > continue)", () => {
    expect(affordanceRank("get started")).toBeLessThan(affordanceRank("continue"));
  });
});
...
describe("chooseForwardAffordance — pick the goal-advancing control, skip decorations", () => {
  it("clicks 'tap to step inside', not the sound/zoom decorations", () => {
    const elements = [
      el({ id: "e0", label: "🔊" }), el({ id: "e1", label: "+" }), el({ id: "e2", label: "−" }),
  ...
  it("returns null when nothing obvious remains (→ hand to the model)", () => {
    const elements = [el({ id: "e0", label: "🔊" }), el({ id: "e1", label: "make a wish" })];
  ...
  it("does not offer an input/select as a forward click", () => {
    const elements = [el({ id: "e0", tag: "input", label: "continue", typable: true })];
    expect(chooseForwardAffordance(elements, "s", new Set())).toBeNull();
  });
});
...
describe("isSensitiveField — reject every credential / payment / personal field", () => {
  it("rejects by input type", () => {
    for (const type of ["password", "email", "tel", "number"]) expect(isSensitiveField({ type })).toBe(true);
  });
  it("rejects by name/placeholder/autocomplete", () => {
    ...
    expect(isSensitiveField({ ariaLabel: "Social Security Number" })).toBe(true);
  });
  it("allows a plain display-name / search field", () => {
    expect(isSensitiveField({ type: "text", name: "displayName", placeholder: "Your name" })).toBe(false);
...
describe("coerceDecision — validates every action against the minted state", () => {
  ...
  const wrap = (action: unknown) => ({ action, expectedChange: "x", goalProgress: "advancing" });

  it("accepts a valid click on a minted id", () => {
    expect(coerceDecision(wrap({ kind: "click_element", elementId: "e0" }), elements)?.action).toEqual({ kind: "click_element", elementId: "e0" });
  });
  it("REJECTS a click on an id that was never minted (no selector smuggling)", () => {
    expect(coerceDecision(wrap({ kind: "click_element", elementId: "e99" }), elements)).toBeNull();
  });
  it("REJECTS typing into a non-typable (password) field", () => {
    expect(coerceDecision(wrap({ kind: "type_text", elementId: "e2", valueKind: "display_name" }), elements)).toBeNull();
  });
  it("accepts typing a synthetic KIND into a typable field, ignores model free-text", () => {
    const d = coerceDecision(wrap({ kind: "type_text", elementId: "e1", valueKind: "display_name", value: "DROP TABLE" }), elements);
    expect(d?.action).toEqual({ kind: "type_text", elementId: "e1", valueKind: "display_name" });
  });
  it("REJECTS an unknown valueKind (only display_name/search/ai_probe)", () => {
    expect(coerceDecision(wrap({ kind: "type_text", elementId: "e1", valueKind: "password" }), elements)).toBeNull();
  });
  it("REJECTS a select option that was not presented", () => {
    expect(coerceDecision(wrap({ kind: "select_option", elementId: "e3", optionValue: "ZZ" }), elements)).toBeNull();
    expect(coerceDecision(wrap({ kind: "select_option", elementId: "e3", optionValue: "US" }), elements)?.action).toEqual({ kind: "select_option", elementId: "e3", optionValue: "US" });
  });
  it("REJECTS a non-allowlisted key, accepts an allowlisted one", () => {
    expect(coerceDecision(wrap({ kind: "press_key", key: "F12" }), elements)).toBeNull();
    for (const key of ALLOWED_KEYS) expect(coerceDecision(wrap({ kind: "press_key", key }), elements)?.action).toEqual({ kind: "press_key", key });
  });
  it("clamps coordinates to 0..100", () => {
    const d = coerceDecision(wrap({ kind: "click_coords", xPct: 999, yPct: -5 }), elements);
    expect(d?.action).toEqual({ kind: "click_coords", xPct: 100, yPct: 0 });
  });
  it("accepts stop(blocked) at a boundary", () => {
    const d = coerceDecision(wrap({ kind: "stop", status: "blocked", reason: "login required" }), elements);
    expect(d?.action).toEqual({ kind: "stop", status: "blocked", reason: "login required" });
  });
  it("returns null for a garbage / unknown action kind", () => {
    expect(coerceDecision(wrap({ kind: "run_javascript", code: "fetch('/x')" }), elements)).toBeNull();
...
describe("actionSignature", () => {
  it("is stable for the same (state, action) and differs across actions/states", () => {
    ...
    expect(actionSignature("s1", a)).not.toBe(actionSignature("s1", { kind: "click_element", elementId: "e2" }));
  });
});
...
describe("decideNextAction — parses + coerces the model decision", () => {
  const view = { url: "https://x/", visibleText: "welcome", elements: [el({ id: "e0", label: "Enter" })] };

  it("returns the coerced action from a valid model reply", async () => {
    const complete = async () => JSON.stringify({ action: { kind: "click_element", elementId: "e0" }, expectedChange: "enters", goalProgress: "advancing" });
    const d = await decideNextAction("talk to yara", view, [], 30, null, { complete });
  ...
  it("retries ONCE on an unusable reply, then returns the valid one", async () => {
    let n = 0;
    const complete = async () => (n++ === 0 ? "not json" : JSON.stringify({ action: { kind: "wait" }, expectedChange: "", goalProgress: "advancing" }));
  ...
  it("gives up (null) after the single retry if still unusable", async () => {
    let n = 0;
    const complete = async () => { n++; return JSON.stringify({ action: { kind: "click_element", elementId: "eNOPE" }, expectedChange: "", goalProgress: "advancing" }); };
    const d = await decideNextAction("g", view, [], 30, null, { complete });
    expect(n).toBe(2);
    expect(d).toBeNull();
  });
