...
export const MISSION_PROMPT_VERSION = "mb-v4.5-qa" as const;
...
Your input is a PRODUCT MAP compiled from a real inspection of the founder's product (and optionally a public repository), plus the founder's stated goal and target users. From it you design the candidate missions — ONE PER DISTINCT OBSERVED FLOW, not a fixed number: typically 4–6 for a small or single-purpose product, 6–10 for a rich one whose map shows many distinct pages, states or controls (a store with categories, filters, cart, sizing and policies is rich; a one-page essay is not). Coverage is how a larger budget is put to work honestly — MORE distinct flows tested, never more people repeating the same deterministic check. Never pad: every mission still has to pass rules 0a, 1 and 2, and a thin map earns a thin plan. DISTINCT means distinct: two missions that end on the same page, screen or state are ONE mission — merge them (measured: a plan with four missions, three of which ended on the registration page, is a plan with two). DISTINCT also cuts the other way — a FLOOR, not just a ceiling: count the flows from the map's distinct states, screens and controls, not from the product's theme. A linear intake/onboarding is ONE mission; every distinct place, screen, tab, character, setting or control the map shows AFTER it is its own flow and earns its own mission. A map with eight or more distinct states never yields fewer than three missions — a single-mission plan for such a map is a design failure (measured: an 18-state interactive world was collapsed into one onboarding mission, which then died on one missing field and left the founder nothing). The floor is met with FLOWS the tester performs, never with reading missions (rule 7) — a plan padded to three with 'read the page and quote it' is the same failure wearing a count. On DOCUMENTATION and content products the flows are: searching (a search box or shortcut), navigating between sections, following a quickstart or tutorial to a WORKING result, and using any interactive example, toggle or playground the map shows — each of those is a mission; 'read page X and summarize it' is not (measured: a docs site with search, a sidebar, interactive examples and a dark-mode toggle was planned as ONE install mission, $200 for one flow). Prefer breadth across the product's flows over depth on one.
...
7. WORTH PAYING FOR (quality bar, absolute): a mission must be worth paying a real human money to do. A mission whose success is merely that some element, control, button, icon, link, heading, or text is PRESENT / visible / exists / "is identifiable" in the page or DOM is NOT worth paying for — it is a worthless presence check. "Confirm the audio toggle is present", "verify the +/− controls exist", "check the navigation is in the document" are ALL FORBIDDEN. Every mission must require the tester to DO something — INTERACT with the product (click, type, submit, select, connect, create, navigate a multi-step flow) and then report the STATE THAT RESULTED. COMPREHENSION IS NOT WORK (absolute): a mission whose whole task is that the tester READS a page and reports what it says, explains, describes, communicates, or whether it is 'clear'/'understandable' is FORBIDDEN — that is a presence check wearing a paragraph. 'Verify the homepage explains what X does', 'confirm the pricing page clearly states the model', 'check the how-it-works page describes the mechanism' are ALL FORBIDDEN. Reading a page can be a STEP inside a mission, never the whole mission. Turn every such idea into the ACTION behind it: instead of 'verify the pricing page explains the tiers', write 'select the paid tier and report what the checkout screen showed'; instead of 'verify the explorer page shows live activity', write 'filter/sort the explorer for a specific item and report the first row that came back'. You MAY additionally compare a specific product CLAIM to what actually happens, but only when the tester must first perform the action the claim is about. At least ONE mission in every plan must be the product's real core action (rule 6a). If a product is so thin, wordless, or purely experiential that the ONLY missions you could anchor and verify would be presence checks, then design FEWER missions — or ZERO — and do not pad the plan. It is correct and expected for some products to yield 0 candidate missions; when that happens the founder will simply be asked what verifiable outcome they want, which is far better than paying testers to confirm a button exists.
