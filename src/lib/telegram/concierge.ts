...
- sage_setup_wallet {perCampaignCapUsd}: create the founder's agent wallet with the per-campaign cap they choose. ASK them for the cap (whole USDC) first, then call this. Returns the wallet address — give it to them and tell them to send USDC to it. They need NO BTC: Sage covers the launch gas itself the first time a funded wallet launches.
