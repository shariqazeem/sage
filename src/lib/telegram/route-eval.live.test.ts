    ...
    async () => {
      ...
      for (const f of fixtures) {
        for (let r = 0; r < RUNS; r++) {
          ...
          console.log(
            `  ${f.id} run${r + 1}/${RUNS}: ${row.called ?? "(no tool)"}${row.viaLookup ? " (via lookup)" : ""}${row.viaSelfCorrect ? " (via self-correct)" : ""} ${mark}` +
      ...
      console.log(`\nP-ROUTE: ${m.correct}/${m.rows - m.providerFailures} correct · misrouted ${m.misrouted}` +
        ...
        ` · viaLookup ${m.viaLookup} · viaSelfCorrect ${m.viaSelfCorrect} · prematureConfirms ${m.prematureConfirms} · providerFailures ${m.providerFailures} · conclusive ${m.conclusive}`);
