...
export async function build() {
  ...
  const body = [
    ...
    K.layer(K.words("One t:operator.", 2.8, { size: 0.07 }) + K.tiles([{ v: "12", k: "wallets" }, { v: "10 / 10", k: "slots taken" }, { v: "11", k: "in one linked cluster" }, { v: "1", k: "person", cls: "no" }], 3.3), { at: 2.7, out: 8.0 }),
    K.layer(K.device({ src: rec("graph"), at: 8.3, out: 13.6, seek: (m.graph ?? 5) + 0.2, cap: "the wallet graph · drawn from the chain, not from a story", right: "sagepays.xyz/graph", ratio: 1, W, focus: { x: 50, y: 48, scale: 1.35 } }), { at: 8.2, out: 13.8 }),
